...
def printout(request, param_list, invoice_type):
    ...
    response[
        "Content-Disposition"
    ] = f'attachment; filename="{invoice_type}_{datetime.now()}.pdf"'
    ...
    if param_list[-1]["kebele"]:
        c.drawString(70, 90, f"Kebele :{param_list[-1]['kebele']}")
        c.drawString(70, 100, f"Received by :{param_list[-1]['receiver']}")
    else:
...
def receipt_printout(request, params_list):
    ...
    c.drawString(
        20,
        70,
        f"DELIVERER/RECEIVER: {params_list[-1]['deliverer_receiver']}    SIGN: _________",
    )

...
class SearchHistory(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        # check if case number exists in Customers 
        regular_treatment = None
        vaccination = None
        ai_service = None
        parasite_treatment = None
        # search for case number in treated animal table, clinical service table, parasite treatment table
        if Treatment.objects.filter(treatment_id__case_number=case_number).exists():
            regular_treatment = Treatment.objects.filter(treatment_id__case_number=case_number)
        if Vaccination.objects.filter(case_number=case_number).exists():
            vaccination = Vaccination.objects.filter(case_number=case_number)
        if ParasiteTreatment.objects.filter(case_number=case_number):
            parasite_treatment = ParasiteTreatment.objects.filter(case_number=case_number)
        if AIService.objects.filter(case_number=case_number):

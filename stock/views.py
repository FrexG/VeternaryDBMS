...
class StockPrintOutView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        selected_kebele = select_kebele_form['receiver'].value()
        ...
        if selected_type == "drug_in":
            ...
            out_drugs = DrugOut.objects.filter(date=selected_date,receiver=selected_kebele)
            if len(out_drugs) > 0:
                out_drugs_list = []
            ...
            out_vaccines = VaccineOut.objects.filter(date=selected_date,receiver=selected_kebele)
            ...
            out_equipments = ClinicalEquipmentOut.objects.filter(date=selected_date,receiver=selected_kebele)

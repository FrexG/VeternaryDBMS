...
class VaccineInView(View):
    def get(self, request):
        ...
        return render(
            request, context=context, template_name="vaccine_in_out/vaccine_in.html"
        )

    def post(self, request):
        vaccine_in_form = VaccineInForm(request.POST)
        if vaccine_in_form.is_valid():
            obj = vaccine_in_form.save()
...
class VaccineOutView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, context=context, template_name="vaccine_in_out/vaccine_out.html"
        )

    def post(self, request):
        vaccine_out_form = VaccineOutForm(request.POST)
        if vaccine_out_form.is_valid():
            obj = vaccine_out_form.save()
            ...
            return render(
                request,
                context=context,
                template_name="vaccine_in_out/vaccine_out.html",
            )
...
class VaccineOutCashDeposit(View):
    ...
    def post(self, request):
        ...
        if vaccine_cash_deposit_form.is_valid():
            ...
            context = {
                "vaccine_out_form": self.vaccine_out_form,
                "vaccine_cash_deposit_form": vaccine_cash_deposit_form,
            }
            return render(
                request,
                context=context,

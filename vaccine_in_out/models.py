...
class VaccineIn(models.Model):
    ...
    receiver = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="vaccine_receiver"
    )
...
class VaccineOut(models.Model):
    ...
    approved_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="vaccine_approved_by"
    )
    ...
    def __str__(self):
        return (
            self.vaccine.vaccine_type + "-" + self.kebele.name + "-" + self.batch_number
        )
...
class VaccineOutCashDeposit(models.Model):
    ...
    deposited_by = models.CharField(max_length=100, default="NOT PROVIDED")
    remaining_amount = models.DecimalField(
        max_digits=8, decimal_places=2, default=000000.00
    )
    confirmed_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="vaccine_out_confirmed_by"
    )
    remark = models.CharField(max_length=200, null=True)

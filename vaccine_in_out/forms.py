    ...
    def clean_quantity(self):
        ...
        if quantity < 0:
            ...
            vaccine_stock = VaccineStock.objects.get(
                vaccine=self.cleaned_data.get("vaccine")
            )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "total": forms.NumberInput(
                attrs={"class": "from-contrl", "type": "hidden"}
            ),
...
class VaccineOutForm(forms.ModelForm):
    ...
    def clean_quantity(self):
        quantity = self.cleaned_data.get("quantity")
        quantity_in_stock = VaccineStock.objects.get(
            vaccine=self.cleaned_data.get("vaccine")
        ).quantity
...
class VaccineOutCashDepositForm(forms.ModelForm):
    def clean_amount(self):
        ...
        previous_payments = VaccineOutCashDeposit.objects.all()
        ...
        total_amount = 0
        for obj in VaccineOut.objects.all():
